...
class AutoFitPreviewBuilder private constructor(
    ...
    init {
        ...
        useCase.setOnPreviewOutputUpdateListener(Preview.OnPreviewOutputUpdateListener {
            ...
            val rotation = getDisplaySurfaceRotation(_viewFinder.display)
            updateTransform(_viewFinder, rotation, it.textureSize, viewFinderDimens)
        })

        // Every time the provided texture view changes, recompute layout
        viewFinder.addOnLayoutChangeListener { view, left, top, right, bottom, _, _, _, _ ->
            val _viewFinder = view as TextureView
            val newViewFinderDimens = Size(right - left, bottom - top)
            Log.d(TAG, "View finder layout changed. Size: $newViewFinderDimens")
            val rotation = getDisplaySurfaceRotation(_viewFinder.display)
            updateTransform(_viewFinder, rotation, bufferDimens, newViewFinderDimens)
        }
